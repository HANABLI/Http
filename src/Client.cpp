...
namespace {
     ...
     */
    bool ParseResponseLine(std::shared_ptr< Http::Client::Response > response, const std::string& responseLine) {
        ...
        if (
            StringUtils::ToInteger(
                responseLine.substr(protocolDelimiter + 1, statusCodeDelimiter - protocolDelimiter - 1),
                statusCodeAsInt
...
namespace Http {
    ...
    auto Client::ParseResponse(const std::string& rawResponse, size_t& messageEnd)-> std::shared_ptr< Response > {
