...
namespace Http
{
    ...
    public:
        ~Server();
        Server(const Server&) = delete;  // Copy Constructor that creates a new object by making a
                                         // copy of an existing object.
        // It ensures that a deep copy is performed if the object contains dynamically allocated
        // resources
        Server(Server&&);  // Move Constructor that transfers resources from an expiring object to a
        ...
        Server& operator=(Server&&);  // Move Assignment Operator: Amove assignment operator
