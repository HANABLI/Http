...
namespace Http
{
    ...
    public:
        ~Client();
        Client(const Client&) = delete;  // Copy Constructor that creates a new object by making a
                                         // copy of an existing object.
        // It ensures that a deep copy is performed if the object contains dynamically allocated
        // resources
        Client(Client&&);  // Move Constructor that transfers resources from an expiring object to a
        ...
        Client& operator=(Client&&);  // Move Assignment Operator: Amove assignment operator
                                      // efficiently transfers resources from one object to another.
